...
static void FileFormat(void)
{
	...
	/* 第一次使用必须进行格式化 */
	result = f_mkfs("0:", FM_ANY, 0, work, sizeof work);//"0:"是卷标，来自于 #define SPI_FLASH		0
	if (result != FR_OK)
	{
		printf("格式化失败 (%s)\r\n", FR_Table[result]);
	}
...
//	uint8_t err = 0;
	...
	s_ucTestSn ++;
	sprintf(TestStr,"好好学习%d",s_ucTestSn);
...
static void  ReadDir(void)
{
	...
	while(1)
	{
		...
		if((result != FR_OK)||(filInfo.fname[0]==0))      //文件路径读取出错，或者读到末尾了
		{
			break;
		}
	...
	{
		read_buff[bw] = 0;
		printf("test1.txt 文件内容 : %s", read_buff);
	}
	else
	{
